# Fliqlo Flip Clock

**Fliqlo Flip Clock**, klasik mekanik flip saat görünümünü modern web teknolojileri ile yeniden tasarlayan bir dijital saat uygulamasıdır. Bu proje, HTML, CSS ve JavaScript kullanılarak geliştirilmiştir ve hem ışık hem de karanlık tema desteği sunar.

## Özellikler <img src="https://github.com/user-attachments/assets/0b0ad12f-4f2b-401c-8676-28801ce2ee1e" alt="Flip Clock Screenshot" width="50" />

- Gerçek zamanlı saat, dakika ve saniye göstergesi
- Flip animasyonu ile klasik flip saat efekti
- Tarih göstergesi (Gün / Ay / Yıl formatında)
- Light ve Dark Mode teması
- Responsive (Mobil Uyumlu) Tasarım

## Teknolojiler

## Teknolojiler 🛠️
| Teknoloji          | Açıklama                             |
| ------------------ | ------------------------------------ |
| **JavaScript**     | Backend dili                         |
| **HTML / CSS**     | Frontend yapısı                      |

## Kurulum

1. Depoyu klonlayın:

```bash
git clone https://github.com/onurcanbircan/Dijital-Saat.git
